...
/* transient overlays (hws, hmenu, pinentries, notifications) are
 * override-redirect and belong above the bar; don't fight them */
...
static void findbattery(void) {
    ...
    while ((de = readdir(dir))) {
        if (!strncmp(de->d_name, "BAT", 3)) {
            snprintf(batpath, sizeof batpath, "/sys/class/power_supply/%s",
                     de->d_name);
            break;
        }
    }
...
static int readcmd(char *const argv[], char *buf, size_t size) {
    fd_set fds;
    struct timeval tv;
    pid_t pid;
    long deadline, left;
...
static int getvolume(const char *target, int *muted) {
...
static void updatedesktop(void) {
    ...
    desknum = -1;
    if (XGetWindowProperty(dpy, root, netcurdesk, 0, 1, False, XA_CARDINAL,
...
static int updatestatus(void) {
    ...
    if ((f = fopen("/proc/stat", "r"))) {
        if (fscanf(f, "cpu %ld %ld %ld %ld %ld %ld %ld %ld", &us, &ni, &sy, &id,
                   &io, &hi, &si, &st) == 8) {
    ...
    if ((f = fopen("/proc/meminfo", "r"))) {
        while (fgets(path, sizeof path, f) && (!mt || !ma || !ms)) {
    ...
    if (mt > 0 && ma > 0 && ma <= mt) {
        ...
        if (tenths > 9999)
            tenths = 9999;
        snprintf(memtxt, sizeof memtxt, "%ld.%ldG", tenths / 10, tenths % 10);
    }
    battxt[0] = '\0';
    if (batpath[0]) {
    ...
    snprintf(stattxt, sizeof stattxt, "%s|%s|%s|%s%d|%s%d|%s",
             ...
             volpct >= 0 ? voltxt : "", volmute, micpct >= 0 ? mictxt : "",
             micmute, clockstr);
...
static int slotw(const char *s, const char *tmpl) {
...
static int statusw(void) {
    int w = 0;

    if (cpupct >= 0)
        w += (int)(cpuw + icgap) + slotw(cputxt, cputmpl) + (int)spacing;
    if (memtxt[0])
        w += (int)(memw + icgap) + slotw(memtxt, memtmpl) + (int)spacing;
    if (volpct >= 0)
        w += (int)(spkw + icgap) + slotw(voltxt, voltmpl) + (int)spacing;
    if (micpct >= 0)
        w += (int)(micw + icgap) + slotw(mictxt, mictmpl) + (int)spacing;
    if (batcap >= 0)
        w += (int)(batw + batnubw + icgap) + textw(battxt) + (int)spacing;
    w += textw(clockstr);
    if (w && !clockstr[0])
        w -= (int)spacing;
    return w;
...
static void layout(void) {
    ...
    if (barw < (int)barh)
        barw = (int)barh;
    XMoveResizeWindow(
        dpy, barwin, mx + mw - barw - 2 * (int)borderpx - (int)padding,
        my + (atbottom ? mh - (int)barh - 2 * (int)borderpx - (int)padding
                       : (int)padding),
        (unsigned int)barw, barh);
    x = (int)hpad + wsw() + (int)(inputw + spacing);
    for (i = 0; i < n; i++) {
        XMoveResizeWindow(dpy, icons[i], x, ((int)barh - (int)iconsize) / 2,
                          iconsize, iconsize);
...
static void drawbattery(int x, int y) {
    ...
    if (fw > 0)
        XFillRectangle(dpy, barwin, gc, x + 2, y + 2, (unsigned int)fw,
                       bath - 4);
}

/* CPU package: outlined die, filled core, pins on all four sides */
static void drawcpu(int x, int y) {
    int i, o;
...
static void drawspeaker(int x, int y, int muted) {
    XPoint cone[4];
    ...
    if (muted) {
        ...
        XDrawArc(dpy, barwin, gc, x + 9, y + 2, 4, 6, -60 * 64, 120 * 64);
        XDrawArc(dpy, barwin, gc, x + 9, y, 6, 10, -60 * 64, 120 * 64);
    }
}

/* capsule + U-shaped stand; a slash across it when muted */
static void drawmic(int x, int y, int muted) {
...
static void drawbar(void) {
    ...
    x = (int)hpad;
    if (desktxt[0]) {
        XftDrawStringUtf8(xd, &xftfg, font, x, ty, (FcChar8 *)desktxt,
                          (int)strlen(desktxt));
        x += textw(desktxt) + (int)spacing;
        XSetForeground(dpy, gc, borderpx_col);
        XDrawLine(dpy, barwin, gc, x, ((int)barh - (int)divh) / 2, x,
                  ((int)barh + (int)divh) / 2);
    ...
    while (*s && textw(s) > (int)inputw - 14)
        s++;
    if (*s)
        XftDrawStringUtf8(xd, &xftfg, font, x + 5, ty, (FcChar8 *)s,
                          (int)strlen(s));
    else if (!inputactive)
        XftDrawStringUtf8(xd, &xftdim, font, x + 5, ty, (FcChar8 *)"$", 1);
    if (inputactive)
        XFillRectangle(dpy, barwin, gc, x + 6 + textw(s), boxy + 4, 2,
                       iconsize - 8);
    ...
    if (cpupct >= 0) {
        ...
        XftDrawStringUtf8(xd, &xftfg, font, x + w - textw(cputxt), ty,
                          (FcChar8 *)cputxt, (int)strlen(cputxt));
        x += w + (int)spacing;
    }
    if (memtxt[0]) {
        ...
        XftDrawStringUtf8(xd, &xftfg, font, x + w - textw(memtxt), ty,
                          (FcChar8 *)memtxt, (int)strlen(memtxt));
        x += w + (int)spacing;
    }
    if (volpct >= 0) {
        ...
        XftDrawStringUtf8(xd, &xftfg, font, x + w - textw(voltxt), ty,
                          (FcChar8 *)voltxt, (int)strlen(voltxt));
        x += w + (int)spacing;
    }
    if (micpct >= 0) {
        ...
        XftDrawStringUtf8(xd, &xftfg, font, x + w - textw(mictxt), ty,
                          (FcChar8 *)mictxt, (int)strlen(mictxt));
        x += w + (int)spacing;
    }
    if (batcap >= 0) {
        drawbattery(x, ((int)barh - (int)bath) / 2);
        x += (int)(batw + batnubw + icgap);
        XftDrawStringUtf8(xd, &xftfg, font, x, ty, (FcChar8 *)battxt,
                          (int)strlen(battxt));
        x += textw(battxt) + (int)spacing;
    }
    if (clockstr[0])
        XftDrawStringUtf8(xd, &xftfg, font, x, ty, (FcChar8 *)clockstr,
                          (int)strlen(clockstr));
}

/* hand the finished command's output to the notification daemon */
static void notifycmd(void) {
    char out[64];
    char *argv[] = {"notify-send",
                    "-a",
                    "htray",
                    cmdname,
                    cmdoutlen ? cmdout : "(no output)",
                    NULL};
...
static void cmdread(void) {
    ...
    if (n < 0 && errno == EINTR)
        return;
    if (n > 0) {
        for (i = 0; i < (size_t)n && cmdoutlen < sizeof cmdout - 1; i++)
...
static void startinput(void) {
    ...
    for (i = 0; i < 100; i++) {
        if (XGrabKeyboard(dpy, barwin, True, GrabModeAsync, GrabModeAsync,
...
static void keypress(XKeyEvent *kev) {
    char c[8];
    KeySym ks = NoSymbol;
...
static void buttonpress(XButtonEvent *bev) {
    int bx = (int)hpad + wsw();

    if (bev->button == Button1 && bev->x >= bx && bev->x < bx + (int)inputw) {
        startinput();
    } else if (inputactive) {
        stopinput();
        drawbar();
...
static void handle(XEvent *ev) {
    ...
    case ConfigureNotify:
        ...
        break;
    case MapNotify:
        if (visible && ev->xmap.event == root && ev->xmap.window != barwin &&
            !ev->xmap.override_redirect)
            XRaiseWindow(dpy, barwin);
        break;
    case PropertyNotify:
        if (ev->xproperty.window == root && ev->xproperty.atom == netcurdesk) {
            updatedesktop();
            layout();
            if (visible)
                drawbar();
...
static void setup(void) {
    ...
    if (fontsize)
        snprintf(fontpat, sizeof fontpat, "%s:pixelsize=%u", fontname,
                 fontsize);
    else
    ...
    XChangeProperty(dpy, selwin, trayorient, XA_CARDINAL, 32, PropModeReplace,
    ...
    swa.event_mask =
        ExposureMask | SubstructureNotifyMask | ButtonPressMask | KeyPressMask;
    barwin = XCreateWindow(
        dpy, root, 0, 0, barh, barh, borderpx, CopyFromParent, CopyFromParent,
        CopyFromParent,
        CWOverrideRedirect | CWBackPixel | CWBorderPixel | CWEventMask, &swa);
    ch.res_name = ch.res_class = "htray"; /* compositor rules match on class */
    ...
    if (!XftColorAllocName(dpy, DefaultVisual(dpy, screen),
        ...
        !XftColorAllocName(dpy, DefaultVisual(dpy, screen),
                           DefaultColormap(dpy, screen), col_border, &xftdim))
